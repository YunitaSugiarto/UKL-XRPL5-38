     ...
     <ul class="list-group">
       <?php foreach ($result as $hasil): ?>
         <li class="list-group-item">
           <h5>Pelanggan : <?php echo $hasil["nama_pelanggan"]; ?>/<?php echo $hasil["id_sewa"]; ?></h5>
           <h5>Daftar Sewa</h5>
           <?php
           $sql2 = "select mobil.*
           from detail_sewa inner join mobil
           on detail_sewa.id_mobil = mobil.id_mobil
           where detail_sewa.id_mobil = '".$hasil["id_sewa"]."'";
           $result2 = mysqli_query($koneksi,$sql2);
            ?>
            <ul>
              <?php foreach ($result2 as $hasil2): ?>
                <li><?php echo $hasil["id_mobil"]."/".$hasil2["nama_pelanggan"]; ?></li>
              <?php endforeach; ?>
            </ul>
         </li>
       <?php endforeach ?>
